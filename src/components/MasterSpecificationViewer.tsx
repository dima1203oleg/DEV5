...
import { 
  ...
  Workflow, Share2, Scale, Activity, ShieldAlert, Binary, Box
} from 'lucide-react';
...
export default function MasterSpecificationViewer() {
  ...
  const [activeTab, setActiveTab] = useState<"chapters" | "constitution" | "graph">("chapters");
  ...
  const handleExportSpecification = () => {
    const specDoc = {
      title: "PREDATOR Analytics - Master Enterprise Program Specification",
      version: "4.0 Enterprise Autonomous",
      chaptersCount: MASTER_SPECIFICATION_CHAPTERS.length,
    ...
    a.href = url;
    a.download = `predator-master-specification-v4.0-${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);

    showToast("Повну Enterprise Специфікацію PREDATOR v4.0 завантажено в JSON", "success");
  };
    ...
    <div className="space-y-6 text-slate-200 font-sans pb-12">
      ...
      <div className="p-6 bg-gradient-to-r from-slate-900 via-indigo-950/80 to-slate-900 border border-indigo-500/30 rounded-3xl relative overflow-hidden shadow-2xl">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-3 py-1 bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 rounded-full text-[11px] font-mono font-bold uppercase tracking-wider flex items-center gap-1.5">
                ...
                Master Enterprise Specification v4.0
              </span>
            ...
            <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">
              Головна Специфікація та Конституція PREDATOR Analytics
            </h2>
            <p className="text-xs sm:text-sm text-slate-300 max-w-4xl leading-relaxed">
              Повний набір з 20 фундаментальних розділів Enterprise Architect, які задають незмінні інваріанти, онтології, графові двигуни, правила безпеки та автономного розвитку платформи для Google Antigravity.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2.5 self-start lg:self-center">
            ...
            >
              ...
              <span>Завантажити Специфікацію (JSON)</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mt-6 pt-4 border-t border-slate-800/80 font-mono text-xs">
          {[
            { id: "chapters", label: "20 Томів Специфікації", icon: Layers },
            { id: "constitution", label: "Майстер Конституція (Invariants)", icon: Scale },
            { id: "graph", label: "Карта Залежностей Архитектури", icon: Network },
          ].map((tab) => {
            const Icon = tab.icon;
