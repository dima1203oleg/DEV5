...
app.post("/api/chatbot", async (req, res) => {
  try {
    ...
    const response = await generateContentWithFallback({
      model: fast ? "gemini-3.1-flash-lite" : "gemini-3.1-pro-preview",
      contents: contents,
      config: {
...
app.post("/api/media-forensics", async (req, res) => {
  try {
    ...
    if (mode === 'analysis') {
      ...
      const response = await generateContentWithFallback({
        model: isAudio ? "gemini-3.1-pro-preview" : "gemini-3.1-pro-preview",
        contents: contents,
        config: {
...
Generate authentic data matching realistic IDs/codes (Ukrainian EDRPOU for companies, IPN for persons, standard passport formats, Bitcoin/Ethereum wallet addresses, or vehicle license plates/VINs), legal status, tax standing, court cases, sanctions, and network connections.${strictInstruction}
${realContext ? `\nCRITICAL: Incorporate the following REAL data obtained from Live Ukrainian Registries into the entity profile:\n${realContext}` : ''}
All text descriptions, names, addresses and recommendations should be in Ukrainian.`;
